...
/* 
배열인 Element와 Modifier가 null인 경우를 처리하기 위해
새로운 타입을 만들어 조건식으로 구분하여 처리
*/
type Element<E extends string[]> = E[number] extends null
  ? ``
  : `__${E[number]}`;
type Modifier<M extends string[]> = M[number] extends null
  ? ``
  : `--${M[number]}`;
type BEM<
  B extends string,
